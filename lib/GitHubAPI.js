...
export class GitHubAPI {
                ...
                this.httpSession.send_and_read_async(message, GLib.PRIORITY_DEFAULT, null, (session, result) => {
                    try {
                        ...
                        if (expectBinary) {
                            ...
                            log(`GitHub API: Binary response received: ${responseData.length} bytes`);
                        } else {
    ...
    async downloadBinaryFile(url, token) {
        log(`GitHub API: Downloading binary file from: ${url}`);
        ...
        if (response.isBinary && response.data instanceof Uint8Array) {
            log(`GitHub API: Successfully downloaded binary file: ${response.data.length} bytes`);
