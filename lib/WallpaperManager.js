...
export class WallpaperManager {
    ...
    async _downloadWallpaperFile(fileInfo, token) {
        ...
        if (fileInfo.download_url) {
            try {
                log(`Wallpaper Manager: Using binary download for ${fileInfo.name}`);
                content = await this.githubAPI.downloadBinaryFile(fileInfo.download_url, token);
                log(`Wallpaper Manager: Binary download successful: ${content.length} bytes`);
            } catch (e) {
                log(`Wallpaper Manager: Binary download failed for ${fileInfo.name}: ${e.message}, trying API URL`);
        ...
        if (fileInfo.name.toLowerCase().match(/\.(jpg|jpeg)$/)) {
            ...
            if (content[0] !== 0xFF || content[1] !== 0xD8 || content[2] !== 0xFF) {
                log(`Wallpaper Manager: WARNING - JPEG header validation failed for ${fileInfo.name}`);
                log(`Wallpaper Manager: File starts with: 0x${content[0].toString(16)} 0x${content[1].toString(16)} 0x${content[2].toString(16)}`);
                log(`Wallpaper Manager: Expected JPEG header: 0xFF 0xD8 0xFF`);
                throw new Error(`Invalid JPEG header for ${fileInfo.name} - file may be corrupted`);
            } else {
                log(`Wallpaper Manager: ✓ JPEG header validation passed for ${fileInfo.name}`);
            }
        }
        ...
        if (fileInfo.name.toLowerCase().match(/\.png$/)) {
            ...
            if (!validPNG) {
                log(`Wallpaper Manager: WARNING - PNG header validation failed for ${fileInfo.name}`);
                throw new Error(`Invalid PNG header for ${fileInfo.name} - file may be corrupted`);
            } else {
                log(`Wallpaper Manager: ✓ PNG header validation passed for ${fileInfo.name}`);
            }
        }
        ...
        try {
            ...
            log(`Wallpaper Manager: Saved wallpaper: ${fileInfo.name} to ${newPath} (${content.length} bytes)`);
        } catch (e) {
        ...
        log(`Wallpaper Manager: ✓ Verified wallpaper file: ${newPath} (${fileSize} bytes)`);
        ...
        try {
            ...
            log(`Wallpaper Manager: ✓ File verification read successful for ${fileInfo.name}`);
        } catch (e) {
            log(`Wallpaper Manager: WARNING - Could not verify file readback for ${fileInfo.name}: ${e.message}`);
        }
    }
        ...
        if (fileResponse.ok) {
            try {
                ...
                if (fileData.content) {
                    ...
                    log(`Wallpaper Manager: API download successful: ${decodedContent.length} bytes`);
    ...
    /**
     * DIAGNOSTIC: Validate downloaded wallpaper files
     * @returns {Object} Validation results
     */
    validateDownloadedWallpapers() {
        const results = {
            valid: [],
            corrupted: [],
            missing: []
        ...
        log('=== WALLPAPER VALIDATION REPORT ===');
        ...
        try {
            ...
            while ((fileInfo = enumerator.next_file(null)) !== null) {
                ...
                const fileSize = fileInfo.get_size();
                const filePath = GLib.build_filenamev([this._wallpaperDir, fileName]);
                const file = Gio.File.new_for_path(filePath);
                
                log(`Validating: ${fileName} (${fileSize} bytes)`);
                ...
                try {
                    ...
                    if (!success || contents.length === 0) {
                        results.corrupted.push({
                            fileName,
                            reason: 'Cannot read file or file is empty',
                            filePath
                        });
                        continue;
                    }
                    ...
                    if (fileName.toLowerCase().match(/\.(jpg|jpeg)$/)) {
                        ...
                        if (contents[0] !== 0xFF || contents[1] !== 0xD8 || contents[2] !== 0xFF) {
                            isValid = false;
                            validationMessage = `Invalid JPEG header: starts with 0x${contents[0].toString(16)} 0x${contents[1].toString(16)} 0x${contents[2].toString(16)} (expected 0xFF 0xD8 0xFF)`;
                        } else {
                            validationMessage = 'Valid JPEG header';
                        }
                    } else if (fileName.toLowerCase().match(/\.png$/)) {
                        // PNG validation
                        const pngHeader = [0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A];
                        for (let i = 0; i < pngHeader.length && i < contents.length; i++) {
                            if (contents[i] !== pngHeader[i]) {
                                isValid = false;
                                validationMessage = `Invalid PNG header at byte ${i}`;
                                break;
                            }
                        }
                        if (isValid) {
                            validationMessage = 'Valid PNG header';
                        }
                    } else {
                        validationMessage = 'Unknown format - header not validated';
                    }
                    ...
                    if (isValid) {
                        ...
                        log(`  ✓ ${fileName}: ${validationMessage}`);
                    } else {
                        results.corrupted.push({
                            fileName,
                            fileSize,
            ...
            log(`Error enumerating wallpaper directory: ${e.message}`);
        }
        
        log(`=== VALIDATION SUMMARY ===`);
        log(`Valid files: ${results.valid.length}`);
        log(`Corrupted files: ${results.corrupted.length}`);
        log(`Missing files: ${results.missing.length}`);
        log('=== END VALIDATION ===');
     ...
     */
    destroy() {
        log('Wallpaper Manager: Starting cleanup');
        ...
        log('Wallpaper Manager: Cleanup complete');
    }
}