...
export class WallpaperManager {
    ...
    validateDownloadedWallpapers() {
        ...
        console.log('=== WALLPAPER VALIDATION REPORT ===');
        
        const wallpaperDirFile = Gio.File.new_for_path(this._wallpaperDir);
        if (!wallpaperDirFile.query_exists(null)) {
            console.error('✗ Wallpaper directory does not exist');
            return results;
        }
        ...
        try {
            ...
            while ((fileInfo = enumerator.next_file(null)) !== null) {
                ...
                console.log(`Validating: ${fileName} (${fileSize} bytes)`);
                ...
                try {
                    ...
                    if (isValid) {
                        ...
                        console.log(`  ✓ ${fileName}: ${validationMessage}`);
                    } else {
                        results.corrupted.push({
                            fileName,
                            fileSize,
                        ...
                        console.error(`  ✗ ${fileName}: ${validationMessage}`);
                    }
            ...
            console.error(`Error enumerating wallpaper directory: ${e.message}`);
        }
    ...
    validateWallpapers() {
        ...
        for (const schema of wallpaperSchemas) {
            try {
                ...
                console.error(`Wallpaper Manager: Error validating schema ${schema}: ${e.message}`);
            }
        }
     ...
     */
    debugWallpaperState() {
        console.log('=== WALLPAPER DEBUG REPORT ===');
        console.log(`Wallpaper directory: ${this._wallpaperDir}`);
        
        // Check if wallpaper directory exists
        const wallpaperDirFile = Gio.File.new_for_path(this._wallpaperDir);
        if (wallpaperDirFile.query_exists(null)) {
            console.log('✓ Wallpaper directory exists');
            ...
            try {
                ...
                if (files.length > 0) {
                    console.log(`✓ Files in wallpaper directory: ${files.join(', ')}`);
                } else {
                    console.warn('✗ Wallpaper directory is empty');
                }
            } catch (e) {
                console.error(`✗ Could not list wallpaper directory: ${e.message}`);
            }
        } else {
            console.error('✗ Wallpaper directory does not exist');
        }
        ...
        console.log(`Wallpaper data map entries: ${this._wallpaperData.size}`);
        for (const [key, data] of this._wallpaperData) {
            console.log(`  ${key} -> ${data.newPath || data.filePath || 'no path'}`);
        ...
        for (const schema of wallpaperSchemas) {
            try {
                ...
                if (!schemaObj) {
                    console.error(`✗ Schema ${schema} not available`);
                    continue;
                }
                ...
                for (const key of keys) {
                    ...
                    try {
                        ...
                        console.error(`✗ Error reading ${schema}.${key}: ${e.message}`);
                    }
                }
            } catch (e) {
                console.error(`✗ Error accessing schema ${schema}: ${e.message}`);
            }
        }
        
        console.log('=== END WALLPAPER DEBUG ===');
    }
