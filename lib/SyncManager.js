...
export class SyncManager {
    ...
    async performSyncOperation(operationType, syncFunction, allowQueue = false) {
        ...
        if (this._isSyncing) {
            log(`Sync Manager: Operation "${operationType}" blocked: another sync in progress`);
            
            if (allowQueue) {
                // Queue the operation for later
                this._syncQueue.push({ operationType, syncFunction });
                log(`Sync Manager: Operation "${operationType}" queued`);
                return Promise.resolve('queued');
            } else {
                // Reject the operation
                log(`Sync Manager: Operation "${operationType}" rejected: not queueing`);
        ...
        log(`Sync Manager: Starting sync operation: ${operationType}`);
        
        try {
            // Perform the actual sync operation
            const result = await syncFunction();
            log(`Sync Manager: Operation completed successfully: ${operationType}`);
            return result;
            
        } catch (error) {
            log(`Sync Manager: Operation failed: ${operationType} - ${error.message}`);
            throw error;
    ...
    _processNextQueuedSync() {
        ...
        log(`Sync Manager: Processing next queued operation: ${nextOperation.operationType}`);
        ...
        this._queueProcessTimeout = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 1000, () => {
            this._queueProcessTimeout = null;
            this.performSyncOperation(nextOperation.operationType, nextOperation.syncFunction, false)
                .catch(error => {
                    log(`Sync Manager: Queued operation failed: ${nextOperation.operationType} - ${error.message}`);
                });
    ...
    async createBackup() {
        ...
        log(`Sync Manager: Creating backup - wallpaper syncing enabled: ${syncWallpapers}`);
        ...
        log(`Sync Manager: Backing up ${allSchemas.length} gsettings schemas`);
        ...
        try {
            ...
            for (const schema of allSchemas) {
                try {
                    ...
                    if (!schemaObj) {
                        log(`Sync Manager: Schema ${schema} not found during backup, skipping`);
                        continue;
                    }
                    ...
                    for (const key of keys) {
                        try {
                            ...
                            log(`Sync Manager: Failed to backup ${schema}.${key}: ${e.message}`);
                        }
                    }
                    
                    if (keyCount > 0) {
                        successfulBackups++;
                        log(`Sync Manager: ✓ Backed up schema ${schema} with ${keyCount} keys`);
                    } else {
                        log(`Sync Manager: ✗ No keys found for schema ${schema}`);
                        delete backup.gsettings[schema];
                    }
                } catch (e) {
                    log(`Sync Manager: Failed to access schema ${schema}: ${e.message}`);
                }
            }
        } catch (e) {
            log(`Sync Manager: Critical error during backup: ${e.message}`);
            throw e;
        }
        
        log(`Sync Manager: Backup complete: ${successfulBackups} schemas successfully backed up`);
        
        if (successfulBackups === 0) {
            log('Sync Manager: WARNING: No schemas were successfully backed up!');
        }
    ...
    async restoreBackup(backup, setMonitoringEnabled) {
        ...
        log(`Sync Manager: Restoring backup from ${backup.timestamp}`);
        
        // Check if wallpaper syncing is enabled
        const syncWallpapers = this.settings.get_boolean('sync-wallpapers');
        log(`Sync Manager: Wallpaper syncing enabled during restore: ${syncWallpapers}`);
        ...
        let monitoringRestoreTimeout = null;
        ...
        try {
            ...
            for (const [schema, keys] of Object.entries(backup.gsettings)) {
                ...
                if (!syncWallpapers && this.wallpaperManager.getWallpaperSchemas().includes(schema)) {
                    log(`Sync Manager: Skipping wallpaper schema ${schema} (wallpaper syncing disabled)`);
                    continue;
                }
                ...
                try {
                    ...
                    if (!schemaObj) {
                        log(`Sync Manager: Schema ${schema} not found during restore, skipping`);
                        continue;
                    }
                    ...
                    for (const [key, value] of Object.entries(keys)) {
                        try {
                            ...
                            log(`Sync Manager: Failed to restore ${schema}.${key}: ${e.message}`);
                        }
                    }
                    
                    log(`Sync Manager: Restored ${restoredKeys} keys for schema ${schema}`);
                } catch (e) {
                    log(`Sync Manager: Failed to access schema ${schema} for restore: ${e.message}`);
                }
            }
            
            log('Sync Manager: GSettings restoration complete');
        } finally {
            // Re-enable change monitoring if it was enabled
            if (setMonitoringEnabled) {
                // Add a small delay to avoid immediate triggers from the restore
                monitoringRestoreTimeout = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 2000, () => {
    ...
    async syncToGitHub() {
        ...
        try {
            log('Sync Manager: Starting GitHub Tree API batch sync');
            ...
            if (await this._shouldUploadContent('config-backup.json', configContent)) {
                ...
                log('Sync Manager: Added config-backup.json to batch');
            } else {
                log('Sync Manager: Skipping config-backup.json - content unchanged');
            }
            ...
            if (treeChanges.length === 0) {
                log('Sync Manager: No changes detected, skipping GitHub commit');
                return;
            }
            
            log(`Sync Manager: Batching ${treeChanges.length} changes into single commit`);
            ...
            log(`Sync Manager: Successfully synced ${treeChanges.length} changes to GitHub using Tree API`);
        } catch (error) {
            log(`Sync Manager: Failed to sync to GitHub: ${error.message}`);
            throw error;
        }
    }
    ...
    async syncFromGitHub() {
        ...
        if (!token || !repo || !username) {
            log('Sync Manager: GitHub credentials not configured, skipping sync from GitHub');
            return;
        }
        ...
        try {
            ...
            log('Sync Manager: Successfully synced from GitHub');
        } catch (error) {
            log(`Sync Manager: Failed to sync from GitHub: ${error.message}`);
            throw error;
        }
    }
        ...
        for (const filePath of filePaths) {
            try {
                ...
                if (!file.query_exists(null)) {
                    log(`Sync Manager: File ${filePath} does not exist, skipping`);
                    continue;
                }
                ...
                if (!success) {
                    log(`Sync Manager: Failed to read file ${filePath}`);
                    continue;
                }
                ...
                if (content.includes('\uFFFD')) {
                    log(`Sync Manager: Skipping ${filePath}: appears to be binary`);
                    continue;
                }
                ...
                if (await this._shouldUploadContent(githubPath, content)) {
                    ...
                    log(`Sync Manager: Added ${githubPath} to batch`);
                } else {
                    log(`Sync Manager: Skipping ${githubPath} - content unchanged`);
                }
                
            } catch (e) {
                log(`Sync Manager: Failed to prepare file ${filePath} for batch: ${e.message}`);
            }
        }
    }
            ...
            try {
                ...
                if (wallpaperContent) {
                    ...
                    if (this._contentHashCache.get(githubPath) !== contentHash) {
                        ...
                        log(`Sync Manager: Added wallpaper ${githubPath} to batch`);
                    } else {
                        log(`Sync Manager: Skipping wallpaper ${githubPath} - content unchanged`);
                    }
                }
            } catch (e) {
                log(`Sync Manager: Failed to prepare wallpaper ${wallpaperKey} for batch: ${e.message}`);
            }
        }
    }
    ...
    async _uploadBatchedChanges(treeChanges, token, username, repo) {
        try {
            ...
            log('Sync Manager: Getting current commit SHA');
            const branchResponse = await this.githubAPI.getBranch(username, repo, token);
            
            let currentCommitSha;
            if (branchResponse.ok) {
                const branchData = JSON.parse(branchResponse.data);
                currentCommitSha = branchData.object.sha;
                log(`Sync Manager: Current commit SHA: ${currentCommitSha}`);
            } else if (branchResponse.status === 404) {
                log('Sync Manager: Repository is empty, will create initial commit');
            ...
            if (currentCommitSha) {
                log('Sync Manager: Getting current tree SHA');
                ...
                if (commitResponse.ok) {
                    ...
                    log(`Sync Manager: Current tree SHA: ${currentTreeSha}`);
                } else {
            ...
            log('Sync Manager: Creating blobs for changed content');
            const treeEntries = [];
            ...
            for (const change of treeChanges) {
                try {
                    ...
                    if (blobResponse.ok) {
                        ...
                        log(`Sync Manager: Created blob for ${change.path}: ${blob.sha}`);
                    } else {
                        log(`Sync Manager: Failed to create blob for ${change.path}: ${blobResponse.status}`);
                    }
                } catch (e) {
                    log(`Sync Manager: Error creating blob for ${change.path}: ${e.message}`);
                }
            }
            ...
            // 4. Create new tree
            log(`Sync Manager: Creating new tree with ${treeEntries.length} entries`);
            const treeData = {
                tree: treeEntries,
            ...
            log(`Sync Manager: Created new tree: ${newTree.sha}`);
            
            // 5. Create commit
            const commitMessage = `Batch sync ${treeEntries.length} files - ${new Date().toISOString()}`;
            log(`Sync Manager: Creating commit: ${commitMessage}`);
            ...
            log(`Sync Manager: Created commit: ${newCommit.sha}`);
            
            // 6. Update branch reference
            log('Sync Manager: Updating branch reference');
            const refData = {
                sha: newCommit.sha,
                force: false
            };
            
            const refResponse = await this.githubAPI.updateRef(username, repo, token, refData);
            ...
            log(`Sync Manager: Successfully uploaded batch of ${treeEntries.length} files in single commit ${newCommit.sha.substring(0, 7)}`);
            
        } catch (error) {
            log(`Sync Manager: Batch upload failed: ${error.message}`);
            throw error;
        }
    }
    ...
    async _downloadFromGitHub(token, username, repo) {
        ...
        if (!response.ok) {
            if (response.status === 404) {
                log('Sync Manager: No backup found on GitHub, skipping restore');
                return null;
            }
        ...
        log('Sync Manager: Downloaded main config from GitHub');
        return backup;
    }
    ...
    async _downloadFilesFromGitHub(token, username, repo) {
        ...
        for (const filePath of filePaths) {
            try {
                ...
                log(`Sync Manager: Downloading file from ${githubPath} to ${filePath}`);
                ...
                if (response.ok) {
                    ...
                    log(`Sync Manager: Successfully restored file ${filePath}`);
                } else if (response.status === 404) {
                    log(`Sync Manager: File ${githubPath} not found in repository, skipping`);
                } else {
                    log(`Sync Manager: Failed to download ${githubPath}: ${response.status}`);
                }
                
            } catch (e) {
                log(`Sync Manager: Failed to download file ${filePath}: ${e.message}`);
            }
        }
    }
     ...
     * @returns {boolean} True if sync operation is in progress
     */
    get isSyncing() {
    ...
    /**
     * Get the number of queued sync operations
    ...
    clearCache() {
        ...
        log('Sync Manager: Cleared content hash cache');
    }
     ...
     */
    destroy() {
        log('Sync Manager: Starting cleanup');
        ...
        log('Sync Manager: Cleanup complete');
    }
}