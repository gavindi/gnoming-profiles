...
export class GoogleDriveProvider extends StorageProvider {
    ...
    async _resolvePathToId(path, credentials, createMissing = false, treatLastAsDir = false) {
        ...
        for (let i = 0; i < segments.length; i++) {
            ...
            // Create missing directory. When treatLastAsDir is true (called from
            // _ensureParentFolders) every segment is a folder; otherwise the last
            // segment is a file that the caller will create.
            if (!isLastSegment || treatLastAsDir) {
                ...
                // Last segment is a file — caller will create it
                return null;
            }
        }
    ...
    async _ensureParentFolders(path, credentials) {
        ...
        const parentId = await this._resolvePathToId(parentPath, credentials, true, true);
