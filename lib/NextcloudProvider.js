                ...
                if (response.ok || response.status === NextcloudProvider.HTTP_CREATED || response.status === NextcloudProvider.HTTP_NO_CONTENT) {
                    ...
                    if (change.path === 'config-backup.json') {
                        ...
                        if (newETag) {
                            ...
                            console.log(`NextcloudProvider: Cached post-upload ETag: ${newETag}`);
                        }
                    }
                } else {
...
<d:propfind xmlns:d="DAV:">
  <d:prop>
     ...
     *
     * HEAD is preferred over PROPFIND because WebDAV servers (SabreDAV/Nextcloud)
     * return 412 Precondition Failed when If-None-Match is used on PROPFIND,
     * whereas HEAD properly returns 304 Not Modified.
     */
    async pollForChanges(credentials) {
        ...
        try {
            const response = await this._request(url, 'HEAD', credentials, null, headers);
            ...
            if (newETag) {
                ...
                if (!cachedETag) {
                    ...
                    console.log(`NextcloudProvider: First poll, cached ETag: ${newETag}`);
                    return { hasChanges: false, etag304: false };
                }

                if (hasChanges) {
                    console.log(`NextcloudProvider: ETag changed from ${cachedETag} to ${newETag}`);
                }
            ...
            console.warn('NextcloudProvider: HEAD returned no ETag, falling back to GET');
            ...
            if (getResponse.etag) {
                ...
                if (!cachedETag) {
                    console.log(`NextcloudProvider: First poll (GET fallback), cached ETag: ${getResponse.etag}`);
                    return { hasChanges: false, etag304: false };
                }
            ...
            console.warn('NextcloudProvider: Server did not return an ETag header on GET either');
